...
interface RecoverabilityTimesheetRow {
  [key: string]: string | undefined
}

// Helper function to get column value, handling both with and without brackets
function getColumnValue(row: RecoverabilityTimesheetRow, possibleNames: string[]): string | undefined {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Delete existing recoverability timesheet data in the date range for this organization
    ...
    if (deleteError) {
      console.error('Error deleting existing recoverability timesheets:', deleteError)
      return NextResponse.json(
        { error: 'Failed to delete existing recoverability timesheets', details: deleteError.message },
        { status: 500 }
      )
    }
    ...
    return new Promise((resolve) => {
      Papa.parse<RecoverabilityTimesheetRow>(fileText, {
        header: true,
        skipEmptyLines: true,
          ...
          try {
            ...
            const records = rows
              .map((row, index) => {
                try {
                  ...
                  const invoicedAmountValue = getColumnValue(row, ['Invoiced Amount', '[Ledger] Invoiced Amount'])
                  const writeOnAmountValue = getColumnValue(row, ['Write On Amount', '[Ledger] Write On Amount'])
                  ...
                  const parsedInvoicedAmount = invoicedAmountValue ? parseFloat(invoicedAmountValue.replace(/[^0-9.-]/g, '')) : null
                  const parsedWriteOnAmount = writeOnAmountValue ? parseFloat(writeOnAmountValue.replace(/[^0-9.-]/g, '')) : null
                  ...
                  return {
                    ...
                    invoiced_amount: parsedInvoicedAmount,
                    write_on_amount: parsedWriteOnAmount,
                  }
                } catch (error: any) {
            ...
            for (let i = 0; i < records.length; i += batchSize) {
              ...
              if (insertError) {
                ...
                return resolve(NextResponse.json(
                  { 
                    error: 'Failed to insert recoverability timesheet data',
            ...
            resolve(NextResponse.json({
              success: true,
              message: `Successfully uploaded ${insertedCount} recoverability timesheet records`,
              insertedCount,
