...
export async function GET(request: NextRequest) {
  try {
    ...
    const cached = getCachedOrgRunner('recoverability-kpi-v1', organizationId, computeRecoverabilityKpi)
    const result = await cached(organizationId, currentYearStart, currentYearEnd, lastYearStart, lastYearEnd, filtersKey)
