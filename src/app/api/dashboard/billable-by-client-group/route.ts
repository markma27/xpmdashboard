...
export async function GET(request: NextRequest) {
  try {
    ...
    const fetchBillableData = async (startDate: string, endDate: string): Promise<any[]> => {
      let allData: any[] = []
      let page = 0
      const pageSize = 1000
      let hasMore = true
      ...
      while (hasMore) {
        const { data: pageData, error: pageError } = await supabase
          ...
          .select('billable_amount, client_group')
    ...
    currentYearData.forEach((record) => {
      ...
      if (!clientGroupMap.has(clientGroup)) {
        clientGroupMap.set(clientGroup, { currentYear: 0, lastYear: 0 })
      }
      
      const existing = clientGroupMap.get(clientGroup)!
    ...
    // Convert to array, sort by current year amount (descending), and take top 10
