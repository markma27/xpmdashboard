...
function getColumnValue(row: InvoiceRow, possibleNames: string[]): string | undefined {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Delete existing invoice data in the date range for this organization
    ...
    if (deleteError) {
      ...
      return NextResponse.json(
        { error: 'Failed to delete existing invoices', details: deleteError.message },
        { status: 500 }
      )
    }
    ...
    return new Promise<NextResponse>((resolve) => {
      Papa.parse<InvoiceRow>(fileText, {
        header: true,
        skipEmptyLines: true,
            ...
            const records = rows
              .map((row, index) => {
                try {
                  ...
                  const invoiceNo = getColumnValue(row, ['Invoice No.', '[Invoice] Invoice No.'])
                  const dateValue = getColumnValue(row, ['Date', '[Invoice] Date'])
                  const clientGroup = getColumnValue(row, ['Client Group(s)', '[Client] Group(s)'])
                  const client = getColumnValue(row, ['Client', '[Client] Client'])
                  const amountValue = getColumnValue(row, ['Amount', '[Invoice] Amount'])
                  const amountInclTaxValue = getColumnValue(row, ['Amount (incl tax)', '[Invoice] Amount (incl tax)'])
                  ...
                  const parsedAmount = amountValue ? parseFloat(amountValue.replace(/[^0-9.-]/g, '')) : null
                  const parsedAmountInclTax = amountInclTaxValue ? parseFloat(amountInclTaxValue.replace(/[^0-9.-]/g, '')) : null
                  ...
                  return {
                    ...
                    client: client || null,
                    amount: parsedAmount,
                    amount_incl_tax: parsedAmountInclTax,
                  }
                } catch (error: any) {
