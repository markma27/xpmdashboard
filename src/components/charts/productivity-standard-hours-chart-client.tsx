  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        let url = `/api/productivity/standard-hours/monthly?organizationId=${organizationId}`
        if (selectedStaff) {
    ...
    return (
      <Card className="shadow-sm border-slate-200">
        <CardHeader className="py-1.5 px-3 flex items-center justify-center bg-gradient-to-r from-blue-50 via-green-100 to-green-50 rounded-t-lg">
          <CardTitle className="text-base font-bold text-slate-800 tracking-tight">Standard Hours - Monthly</CardTitle>
        </CardHeader>
        <CardContent>
