...
export function RecoverabilityClientGroupsTable({ 
    ...
    return (
      <Card className="shadow-sm border-slate-200">
        <CardHeader className="py-1.5 px-3 flex items-center justify-center bg-gradient-to-r from-blue-50 via-green-100 to-green-50 rounded-t-lg">
          <CardTitle className="text-base font-bold text-slate-800 tracking-tight">Recoverability by Client Group</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Recoverability by Client Group')
    XLSX.writeFile(workbook, `recoverability_by_client_group_${excelTimestamp()}.xlsx`)
  }
          ...
          <table className="w-full border-collapse text-[12px]">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              <tr className="border-t-2 border-slate-200 font-bold bg-slate-50/80 rounded-b-lg">
                ...
                <td className="p-2 border-r"></td>
                <td className={`p-2 text-right border-r max-w-[140px] ${totalCurrentYear < 0 ? 'text-red-600' : ''}`}>
